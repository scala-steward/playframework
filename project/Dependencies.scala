...
object Dependencies {
  ...
  val pekkoHttpVersion     = sys.props.getOrElse("pekko.http.version", "2.0.0-M1+236-096a7671-SNAPSHOT")
  ...
  val jpaDeps = Seq(
    ...
    "org.hibernate.orm"   % "hibernate-core"          % "7.4.7.Final" % "test",
    "org.hibernate.orm"   % "hibernate-scan-jandex"   % "7.4.7.Final" % "test"
  )
  ...
  val playdocWebjarDependencies = Seq(
    "org.webjars" % "jquery"   % "4.0.0"        % "webjars",
  ...
  val testDependencies = Seq(junit, junitInterface, guava, logback) ++ Seq(
    ...
    "commons-io"              % "commons-io"              % "2.22.0", // explicitly bump commons-io to newer version for fluentlenium and htmlunit
